...
import { MessageField } from '../../sendMessage/ui/MessageField'
import styles from './Chat.module.css'

export function Chat() {
	const { userId } = useAuthStore()
	const { currentChatId, currentChat } = useChatStore()
							...
							return (
								<React.Fragment key={index}>
									<Message
										...
										isEditing={element.createdDate != element.updatedDate}
										time={dayjs(element.createdDate).format('HH:mm')}
										message={element.text || 'default message'}
										isSelf={sender?.id == userId}
									/>
								</React.Fragment>
							)
						})}
